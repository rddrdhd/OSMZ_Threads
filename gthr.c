...
bool gtyield(void) {
    ...
    if (gtcur->st != Unused) {			// switch current to Ready and new thread found in previous loop to Running
        gtcur->st = Ready;
        if (gtcur->t_start.tv_usec > 0) {
            long diff = time_diff(current_time, gtcur->t_start);
            gtcur->run_time.sum += diff;
            if (p->run_time.min == 0) 
                p->run_time.min = diff;
            if (p->run_time.min > diff) 
                p->run_time.min = diff;
            if (p->run_time.max < diff) 
                p->run_time.max = diff;
        }
...
void handle_sigint(int sig) {
    struct gt* p;
    printf("\n\n|\t |\t\tRUN TIME (ms)\t\t|\t\tWAIT TIME (ms)\t\t|\n");
    printf("| THREAD |--------------------------------------|---------------------------------------|\n");
    printf("|   ID\t |      SUM\t|  AVG\t|  MIN\t|  MAX\t|      SUM\t|  AVG\t|  MIN\t|  MAX\t|\n");
    printf("|--------|--------------|-------|-------|-------|---------------|-------|-------|-------|\n");
    for (int i = 1; i < MaxGThreads; i++) {
        p = &gttbl[i];
        printf("| %d\t | %06.3f\t| %0.3f\t| %0.3f\t| %0.3f\t| %06.3f\t| %0.3f\t| %0.3f\t| %0.3f |\n",
            i, 
